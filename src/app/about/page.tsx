...
export const metadata: Metadata = {
  title: "О компании",
  description:
    "ОкнаКредит — производитель металлопластиковых окон с 2009 года. Профиль GEALAN, фурнитура ROTO FRANK. Более 2000 выполненных объектов.",
};
      ...
      <section className="bg-brand-navy text-white py-20 md:py-28">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="mt-5 text-white/70 text-lg max-w-2xl leading-relaxed">
            ОкнаКредит — прямой производитель металлопластиковых окон и дверей. Работаем без посредников, поэтому вы платите только за изделие и работу — без дилерской наценки.
          </p>
          <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-6 max-w-2xl">
            {[
              ...
              <div key={label}>
                <p className="text-3xl font-bold text-brand-gold">{val}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <div className="space-y-4 mt-6 text-brand-gray leading-relaxed">
              <p>
                ОкнаКредит появился из простой идеи: зачем переплачивать дилерам, если можно купить окно напрямую у того, кто его делает? В 2009 году мы запустили собственное производство на немецком оборудовании с профилем GEALAN и фурнитурой ROTO FRANK.
              </p>
              <p>
          ...
          <div className="bg-brand-navy/5 rounded-2xl h-80 flex items-center justify-center">
            <svg className="w-24 h-24 text-brand-navy/20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="relative border-l-2 border-brand-gold/30 pl-8 space-y-8">
              {milestones.map((m) => (
                <div key={m.year} className="relative">
                  <div className="absolute -left-10 top-0.5 w-4 h-4 rounded-full bg-brand-gold" />
                  <p className="text-brand-gold font-bold text-sm mb-1">{m.year}</p>
                  <p className="text-brand-navy font-medium">{m.event}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {team.map((t) => (
            <div key={t.name} className="card p-6 text-center">
              <div className="w-16 h-16 bg-brand-navy/10 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg className="w-8 h-8 text-brand-navy/40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
              </div>
              <h3 className="font-heading font-semibold text-brand-navy text-lg">{t.name}</h3>
              <p className="text-brand-gold text-xs font-semibold uppercase tracking-wider mt-1">{t.role}</p>
          ...
          <div className="max-w-2xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-4">
            {certifications.map((c) => (
              <div key={c} className="flex items-start gap-3 bg-brand-cream rounded-xl p-4">
                ...
                <span className="text-brand-navy text-sm font-medium">{c}</span>
              </div>
            ))}
          </div>
        </div>
